...
module.exports = function(app) {
  app.get('/', function (req, res) {
    //判斷是否是第一頁，並把請求的頁數轉換成 number 類型
    var page = req.query.p ? parseInt(req.query.p) : 1;
    //查詢並返回第 page 頁的 10 篇文章
  ...
  app.post('/reg', function (req, res) {
    ...
    User.get(newUser.name, function (err, user) {
      ...
      newUser.save(function (err, user) {
        if (err) {
          req.flash('error', err);
          return res.redirect('/reg');//註冊失敗返回註冊頁
        }
  ...
  app.post('/login', function (req, res) {
    ...
    User.get(req.body.name, function (err, user) {
      if (!user) {
        ...
        return res.redirect('/login');//用戶不存在則轉移到登入頁
      }
      //檢查密碼是否一致
      if (user.password != password) {
        req.flash('error', '密碼錯誤!'); 
        return res.redirect('/login');//密碼錯誤則切換到登入頁
      }
      //用戶名密碼都符合後，將用戶信息存入 session
      req.session.user = user;
      req.flash('success', '登入成功!');
      res.redirect('/');//登入成功後切回主頁面
    });
  });
